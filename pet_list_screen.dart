...
class _PetListScreenState extends State<PetListScreen> {
  ...
  Future<void> fetchUserPets() async {
    final url = Uri.parse(
      'http:// 192.168.1.105/pet_api/get_user_pets.php?user_id=${widget.userId}',
    );
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ...
          : pets.isEmpty
          ? const Center(
              child: Text("ยังไม่มีข้อมูลสัตว์เลี้ยง กรุณาเพิ่มข้อมูลก่อน"),
            )
          : ListView.builder(
  ...
  Widget _buildPetCard(Map pet) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text("ประเภท: ${pet['pet_type']} "),
        trailing: const Icon(
